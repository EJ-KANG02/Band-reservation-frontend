...
export default function OfficerSignupPage() {
  ...
  const handleChange = (field, value) => {
    // 숫자만 허용 필드
    if ((field === 'batch' || field === 'studentId') && value !== '') {
  ...
  const validate = () => {
    ...
    const next = {
      nickname:  !form.nickname.trim() ? '필수 입력 항목입니다.' : '',
      name:      !form.name.trim()     ? '필수 입력 항목입니다.' : '',
      studentId: !form.studentId.trim() ? '필수 입력 항목입니다.' : '',
      batch:     !form.batch.toString().trim()
                   ? '필수 입력 항목입니다.'
                   : (!Number.isInteger(batchNum) || batchNum <= 0)
                     ? '올바른 기수를 입력해주세요. (숫자만)'
                     : '',
      position:  !form.position ? '직책을 선택해주세요.' : '',
    }
    setErrors(next)
  ...
  return (
    <div className="min-h-screen bg-black flex flex-col px-6">
      ...
      <h1 className="text-white text-2xl font-bold mb-1">회장단 가입</h1>
      ...
      <div className="flex flex-col gap-5 flex-1">
        ...
        {/* 직책 선택 */}
        <div className="flex flex-col gap-1.5">
          <label className="text-zinc-400 text-xs font-medium">직책</label>
          <div className="relative">
            <select
              value={form.position}
              onChange={e => handleChange('position', e.target.value)}
              className={`w-full bg-zinc-900 px-4 py-3.5 rounded-xl outline-none text-sm
                          appearance-none border transition-colors
                          ${form.position ? 'text-white' : 'text-zinc-600'}
                          ${errors.position
                            ? 'border-red-500/60'
                            : 'border-transparent focus:border-zinc-600'
                          }`}
            >
              ...
              {POSITIONS.map(p => (
                <option key={p.value} value={p.value} className="text-white bg-zinc-900">
                  {p.label}
                </option>
              ))}
            </select>
          ...
          {errors.position && (
            <p className="text-red-400 text-xs pl-1">{errors.position}</p>
          )}
        </div>
